...
public class TransacaoService {
    ...
    private Transacao processarEnvio(Transacao transacao) {
        ...
        // Validar se professor e aluno são da mesma instituição
        if (professor.getInstituicao() == null || aluno.getInstituicao() == null) {
            throw new IllegalArgumentException("Professor e aluno devem estar vinculados a uma instituição");
        }
        
        if (!professor.getInstituicao().getId().equals(aluno.getInstituicao().getId())) {
            throw new IllegalArgumentException("Professor e aluno devem pertencer à mesma instituição de ensino");
        }
